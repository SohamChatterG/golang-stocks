...
const OrdersTable: React.FC<OrdersTableProps> = ({ refreshTrigger }) => {
    ...
    return (
        <div className="bg-white dark:bg-slate-900 rounded-lg shadow-md p-6">
            ...
            {error && (
                <div className="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 px-4 py-3 rounded-md mb-4">
                    {error}
                </div>
            )}
                    ...
                    <div className="block md:hidden">
                        <div className="space-y-3">
                            {filteredOrders.map((order) => (
                                <div key={order.id} className="bg-white dark:bg-slate-800 rounded-lg p-4 border border-gray-200 dark:border-slate-700">
                                    {/* Header: Stock info */}
                                    <div className="flex items-center justify-between mb-3">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="font-bold text-gray-900 dark:text-gray-100">{order.symbol}</div>
                                        ...
                                        <div className="flex flex-col items-end gap-1">
                                            <span className={`px-2 py-1 rounded-full text-xs font-semibold uppercase ${getSideBadgeColor(order.side)}`}>
                                                {order.side}
                                            </span>
                                            <span className={`px-2 py-1 rounded-full text-xs font-semibold uppercase ${getStatusBadgeColor(order.status)}`}>
                                                {order.status}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-3 gap-2 text-sm">
                                        <div>
                                            ...
                                            <div className="font-semibold text-gray-900 dark:text-gray-100 capitalize">{order.orderType || 'limit'}</div>
                                        </div>
                                        <div>
                                            <div className="text-gray-500 dark:text-gray-400 text-xs">Quantity</div>
                                            <div className="font-semibold text-gray-900 dark:text-gray-100">{order.quantity}</div>
                                        </div>
                                        <div>
                                            <div className="text-gray-500 dark:text-gray-400 text-xs">Price</div>
                                            <div className="font-semibold text-gray-900 dark:text-gray-100">${order.price.toFixed(2)}</div>
                                        </div>
                                    </div>
                                </div>
                            ))}
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span
                                                className={`px-3 py-1 rounded-full text-xs font-semibold uppercase ${order.orderType === 'market'
                                                    ? 'bg-blue-100 text-blue-800 border border-blue-300 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-800'
                                                    : 'bg-purple-100 text-purple-800 border border-purple-300 dark:bg-purple-900/30 dark:text-purple-300 dark:border-purple-800'
                                                    }`}
                                            >
